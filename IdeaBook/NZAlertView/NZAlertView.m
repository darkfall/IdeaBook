...
static float const kAlertDuration = 3.f;
...
- (void)showWithCompletion:(NZAlertViewCompletion)completion
{
    ...
    if(_blurParameter > 0.f) {
        NSData *imageData = UIImageJPEGRepresentation(screenshot, .0001);
        UIImage *blurredSnapshot = [[UIImage imageWithData:imageData] blurredImage:_blurParameter];
    
        self.backgroundView.image = blurredSnapshot;
    } else {
        self.backgroundView.image = screenshot;
    }
